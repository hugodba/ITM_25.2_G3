...
class Circuit:
    ...
    def create_element(self, line:str, num_nodes:int):
        ...
        if line[0].startswith("R"):
            ...
            indutor = Inductor(self, line[0], int(line[1]), int(line[2]), float(line[3]), float(line[4].split("=")[1]) if len(line) > 4 else 0.0, num_nodes + self.extra_lines + 1)
            ...
            fctc = VoltageControlledVoltageSource(self, line[0], int(line[1]), int(line[2]), int(line[3]), int(line[4]), float(line[5]), num_nodes + self.extra_lines + 1)
            print(fctc, "adicionado com sucesso")
            return fctc
        
        elif line[0].startswith("V"):
            voltage_source = VoltageSource(self, line[0], int(line[1]), int(line[2]), line[3], float(line[4]), num_nodes + self.extra_lines + 1)
